...
```{r sc1}
# Sample 30 beads 5 times
# replicate 1:5
#samp: list-column

# Create a new tibble when it says to
# Sample will work on a vector, sample_n can work on a tibble
random_samples <- 
...
tibble(replicate = 1:1000,
  ...
  mutate(more = ifelse(numb_red > numb_blue, TRUE, FALSE)) %>%
  summarize(prob = sum(more)/n())
  # group_by(more) %>%
  # count()
...
```{r sc3}
tibble(num_red = 0:1000) %>%
  mutate(red_paddle = map_int(num_red, ~ rbinom(n = 1, size = 25, prob = ./1000)))
